...
class MainActivity : AppCompatActivity() {
    ...
    private fun showManualGuide() {
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("입력 가이드 (서식 유지)")
            .setMessage(
                "안드로이드는 '길게 눌러 붙여넣기'로는 볼드·색·형광이 평문으로 떨어질 수 있어요\n" +
                    "(PC는 됩니다 — WebView 클립보드 제약). 서식까지 그대로 넣으려면\n" +
                    "앱의 입력 버튼을 쓰는 게 가장 확실합니다.\n\n" +
                    "■ 권장 (서식 그대로 — 클립보드 안 거침)\n" +
                    "① 상단 '제목 입력' → 제목이 에디터에 바로 들어갑니다\n" +
                    "② 상단 '내용 입력' → 본문이 볼드·색·형광까지 함께 들어갑니다\n" +
                    "    (앱이 에디터에 execCommand 로 직접 서식 적용)\n\n" +
                    "■ 폴백 (평문이라도 빠르게)\n" +
                    "③ '제목 복사'/'내용 복사' 후 해당 칸을 길게 눌러 '붙여넣기'\n\n" +
                    "④ 사진은 '사진 입력' 버튼으로 올리세요"
            )
        ...
        web.webViewClient = object : WebViewClient() {
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url.contains("m.blog.naver.com") && !isPublishedUrl(url)) {
                    // 안드로이드 클립보드 '붙여넣기'는 WebView 제약으로 text/html 이 평문으로 떨어져
                    // 볼드·색·형광이 사라지는 경우가 많다(PC는 됨). → 클립보드 대신 에디터 DOM 에
                    // execCommand(bold/foreColor/hiliteColor)로 '직접' 서식을 주입한다(이게 갤럭시에서 확실).
                    dbg("모바일 글쓰기 페이지: ${shortUrl(url)} — 서식 직접 주입 시도")
                    if (!filled) injectFill()
                    setStatus("모바일 글쓰기 — 서식 자동 입력 중… (안 되면 상단 '내용 입력' 버튼)")
        ...
        androidx.appcompat.app.AlertDialog.Builder(this)
            ...
            .setMessage(
                "제목·본문이 클립보드에 준비됐어요(본문은 서식 포함).\n\n" +
                    ...
                    "※ 안드로이드 외부 앱 붙여넣기는 볼드·색이 평문으로 떨어질 수 있어요.\n" +
                    "   서식까지 확실히 넣으려면 '📤 블로그 포스팅'(앱 내 글쓰기)을 쓰면\n" +
                    "   에디터에 볼드·색·형광이 직접 적용됩니다."
            )
            .setPositiveButton("확인", null)
            .show()
    }
